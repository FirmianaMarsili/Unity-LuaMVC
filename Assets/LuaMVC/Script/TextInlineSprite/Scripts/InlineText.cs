...
public class InlineText : Text, IPointerClickHandler
{
    ...
    private static readonly Regex _InputTagRegex = new Regex(@"\[(\-{0,1}\d{0,})#(.+?)\]", RegexOptions.Singleline);   //匹配规则      正则表达式 学习 https://www.cnblogs.com/zery/p/3438845.html
    //文本表情管理器
	...
	#region 超链接
	[System.Serializable]   //自定义事件
    public class HrefClickEvent : UnityEvent<string,int> { }   //自定义事件
    //点击事件监听
    public HrefClickEvent OnHrefClick = new HrefClickEvent();    //自定义事件
    // 超链接信息列表  
        ...
        foreach (Match match in _InputTagRegex.Matches(text))
        {
            ...
            if (_tempID <0 )  /*这里修改表情和超链接的识别ID*/
            {
                ...
                var hrefInfo = new HrefInfo
                {
                    ...
                    id = Mathf.Abs(_tempID),  /*这里可以修改超链接id*/
    ...
    public void OnPointerClick(PointerEventData eventData)
    {
        ...
        foreach (var hrefInfo in _ListHrefInfos)
        {
            ...
            for (var i = 0; i < boxes.Count; ++i)
            {
                ...
                {
                    OnHrefClick.Invoke(hrefInfo.name, hrefInfo.id);  //注册事件
                    return;
                }
            }
        }
